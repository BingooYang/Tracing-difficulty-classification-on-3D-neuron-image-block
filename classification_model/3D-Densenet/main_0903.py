...
def Net():
    ...
    if(not debug):
        ...
        save_para.model_scalar('app2 background', str(bck))
        save_para.model_scalar('model:', '/home/zhang/disk2/001yangbin/001vaa3d/plot_network/densenet_2_4_8_8')
        save_para.model_scalar('train path:', data_path_train)
        save_para.model_scalar('test path:', data_path_test)
        save_para.model_scalar('layer size:', str(layer_size))
        save_para.model_scalar('growth radio:', growth_radio)
        save_para.model_scalar('pre layers:', 'no')
    ...
    for n in range(ave_num):
        ...
        for e in range(epoch):
            model.train()
            #每10个epoch降低10倍
            # model_lr = model_lr / math.ceil((e+10)/10)
        ...
        model_save_path = os.getcwd() + '/model/' + 'Densnet_2_2_4_4_ave_' + str(n) + '_' + time.strftime("%Y-%m-%d %H:%M:%S",
                                                                                      time.localtime()) + '.pth'
        # 保存模型
        torch.save({
            'epoch': epoch,
