...
from resnet_SE import generate_model
...
def SEnet():
    ...
    bck = True
    debug = False
    print('start SEresnet_1219')

    ### 测试集和训练集分别为一个文件夹
    # data_path_train = "/home/zhang/disk2/001yangbin/002vaa3d_img_samples/data_agmentation/original_image_train/"
    # data_path_test = "/home/zhang/disk2/001yangbin/002vaa3d_img_samples/data_agmentation/original_image_test_2000/"
    ...
    data = GetSample(root=data_path_train,background=bck)
    ...
    if(not debug):
        ...
        save_para.model_scalar('model_layers_num', model_layers_num)
        save_para.model_scalar('model_batch_size', model_batch_size)
        save_para.model_scalar('model_num_workers', model_num_workers)
        save_para.model_scalar('test_set_ratio', test_set_ratio)
        save_para.model_scalar('epoch', epoch)

        save_para.model_scalar('batch normalization', 'yes')
        save_para.model_scalar('app2 cut background', str(bck))
        save_para.model_scalar('se model:', str(True))
        save_para.model_scalar('dropout:', str(dropout))
        save_para.model_scalar('optimizer:', 'Adam')

        save_para.model_scalar('model:', 'SEresnet_layer_10_no_premaxpooling_1220')
    ...
    for n in range(ave_num):
        ...
        model = generate_model(10, senet=True, dropout_radio=dropout).to(device)
    ...
    model_save_path = os.getcwd() + '/model/' + 'SE_resenet_1219_10' + time.strftime("%Y-%m-%d %H:%M:%S",time.localtime()) + '.pth'
    #保存模型
    torch.save({
        'epoch': epoch,
...
def main():
    ...
    print('Procedure start time：{}'.format(now))
