...
def resnet_1():
    ...
    if(not debug):
        ...
        save_para = SavePara(save_para_path,s_name='ave_5_')
        ...
        save_para.model_scalar('kernel size', (3,3,3))
        ...
        save_para.model_scalar('ave_n:', ave_num)
    ...
    for n in range(ave_num):
        ...
        model = generate_model(model_layers_num, dropout_radio=dropout).to(device)
        ...
        for e in range(epoch):
            ...
            for batch_idx, (test_data, bag_label,image_name) in enumerate(test_loader):
                # bag_label = torch.from_numpy(test_label)
        ...
        model_save_path = os.getcwd() + '/model/' + 'Resnet_10_ave_' + str(n) + '_' + time.strftime("%Y-%m-%d %H:%M:%S",
                                                                                 time.localtime()) + '.pth'
        # 保存模型
        torch.save({
            'epoch': epoch,
